...
namespace Updater.Native
{
    public class Windows : OS.IBackend
    {
        ...
        {
            Process.Start(new ProcessStartInfo($"{Program.Name}.exe") { UseShellExecute = false, RedirectStandardOutput = true, CreateNoWindow = true, WorkingDirectory = workingDir ?? Directory.GetCurrentDirectory() });
        }
        ...
        public async Task Start(string[] args)
        {
            ...
            if (args.Any())
            {
                ...
                if (!await Update(args) || !File.Exists($"{Program.Name}.exe"))
                    return;
